...
:
    ...
    volumeTotal_(0.0),
    massTotal_(0.0),
	minNParticle_(this->coeffDict().lookupOrDefault("minNParticle", 1.0)), //added
    massFlowRate_(owner.db().time(), "massFlowRate"),
    massInjected_(this->template getModelProperty<scalar>("massInjected")),
...
:
	...
	minNParticle_(im.minNParticle_), //added
        ...
        for (label parcelI = 0; parcelI < newParcels; parcelI++)
        {
            if (validInjection(parcelI))
            {
                ...
                if (cellI > -1)
                {
                    ...
                    if (pPtr->nParticle() >= minNParticle_) //modified
                    {
                        parcelsAdded++;
