...
)
{
	...
	td.updates() = TrackData::motion; //added1
	td.updateParticleStatistics(); //(*this); //added1

    td.part() = TrackData::tpLinearTrack;
    CloudType::move(td,  deltaT);

	//It is not needed anymore when kinematicRotCloud is used as superclass
...
:
	CloudType(cloudName, rho, U, mu, E, g, false),
    constProps_(this->particleProperties()),
    collisionModel_(NULL)
{
    constructorComplement(readFields);
}


template<class CloudType> //added1
void Foam::CollidingCloud<CloudType>::constructorComplement(bool readFields)
{
...
:
    ...
    collisionModel_(NULL)
{
    /*if (this->solution().steadyState()) //changed
    {
        FatalErrorIn
        (
            "Foam::CollidingCloud<CloudType>::CollidingCloud"
