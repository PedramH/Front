...
:
    ...
    f_(vector::zero),
    //angularMomentum_(vector::zero), //changed
    torque_(vector::zero),
    collisionRecords_()
{
    if (readFields)
    {
        ...
        else
        {
            ...
            (
              ...
              + sizeof(f_)
              //+ sizeof(angularMomentum_) //changed
              + sizeof(torque_)
            );
        }
...
void Foam::CollidingParcel<ParcelType>::readFields(CloudType& c)
{
    ...
    /*IOField<vector> angularMomentum //changed
    (
    ...
    forAllIter(typename CloudType, c, iter)
    {
        ...
        p.f_ = f[i];
        //p.angularMomentum_ = angularMomentum[i]; //changed
        p.torque_ = torque[i];
...
void Foam::CollidingParcel<ParcelType>::writeFields(const CloudType& c)
{
    ...
    forAllConstIter(typename CloudType, c, iter)
    {
        ...
        f[i] = p.f();
        //angularMomentum[i] = p.angularMomentum(); //changed
        torque[i] = p.torque();
...
)
{
    if (os.format() == IOstream::ASCII)
    {
        os  << static_cast<const ParcelType&>(p)
            << token::SPACE << p.f()
            //<< token::SPACE << p.angularMomentum() //changed
    ...
    else
    {
        ...
        (
          ...
          + sizeof(p.f())
          //+ sizeof(p.angularMomentum()) //changed
          + sizeof(p.torque())
        );
